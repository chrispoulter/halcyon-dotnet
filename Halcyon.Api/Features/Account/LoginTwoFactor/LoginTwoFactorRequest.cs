...
public class LoginTwoFactorRequestValidator : AbstractValidator<LoginTwoFactorRequest>
{
    public LoginTwoFactorRequestValidator()
    {
        ...
        RuleFor(x => x)
            .Must(x =>
                !string.IsNullOrWhiteSpace(x.Code) || !string.IsNullOrWhiteSpace(x.RecoveryCode)
            )
        ...
        When(
            ...
            () =>
            {
                RuleFor(x => x.Code!).Length(6).Matches("^[0-9]{6}$").WithName("Two-factor code");
            }
        );

        When(
            x => !string.IsNullOrWhiteSpace(x.RecoveryCode),
            () =>
            {
